...
MAX_PAGES = 5  # Each "Load more" adds ~20 jobs
    ...
    async with async_playwright() as p:
        ...
        async def handle_response(response):
            ...
            try:
                ...
                if page_jobs:
                    ...
                    print(f"Meta: captured {len(page_jobs)} jobs (total: {len(all_jobs)})")
            except Exception:
                pass

        page.on("response", handle_response)

        print("Meta: loading jobs page...")
        ...
        for i in range(MAX_PAGES - 1):
            try:
                ...
                await btn.click()
                await page.wait_for_timeout(3000)
                print(f"Meta: clicked 'Load more' ({i + 2}/{MAX_PAGES})")
            except Exception:
                print("Meta: no more pages.")
                break

        await browser.close()

    # Dedupe and filter to US
    seen = set()
    jobs = []
